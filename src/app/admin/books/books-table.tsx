...
export function BooksTable({ data, pageCount, categories }: BooksTableProps) {
  ...
  const columns = useMemo(() => getBookColumns(categoryOptions), [categoryOptions]);
  ...
  async function handleReorder(orderedIds: string[]) {
    ...
    setRows((prev) => {
      ...
      const reordered = orderedIds
        .map((id) => byId.get(id))
        .filter((row): row is Book => Boolean(row));
  ...
  return (
    <DataTable
      table={table}
      onRowClick={(book) => router.push(`/admin/books/${book.id}/edit`)}
